...
function test_goal() {
	...
	if [[ "${2}" == "jacobi" ]]; then
		for (( i = 0; i <= $jacobi; i++ )); do
			...
			individual_test $1 $2 jacobi_$i.txt none
			echo
		done
	elif [[ "${2}" == "spk" ]]; then
		for (( i = 2; i <= 6; i++ )); do
		...
		for (( i = 4; i <= 7; i++ )); do
			...
			individual_test $1 $2 spk_$i.txt none
			echo
		done
		for (( i = 8; i <= 8; i++ )); do
			echo -n "${1^^}: ${2^^}: ${testers_path}/spk_${i}.txt: "
			individual_test $1 $2 spk_$i.txt spk_5
			echo
		done
		for (( i = 9; i <= 9; i++ )); do
			echo -n "${1^^}: ${2^^}: ${testers_path}/spk_${i}.txt: "
...
function individual_test() {
	...
	if [[ "${4}" == "spk_5" ]]; then
		python3 spkmeans.py 5 $2 $testers_path/$3 &> $output_file
	elif [[ "${1}" == "py" ]]; then # if we are testing the python interface
